...
<!-- 			상품 카테고리 메뉴바 -->
			...
			<br>	          	
			<br>	          	
			<h3  style="text-align:center;"><b>진행중인</b> 경매내역</h3>   
			...
			<br>     	
				<table class="table table-hover">
					...
					<c:forEach items="${underwayAuctionList}" var="GoodsVO">
							<tbody>
								<tr>
				...
				<ul class="pagination justify-content-center">
					...
					<c:if test="${prev}">
						<li class="page-item"><a class="page-link"
							href="/admin/underwayAuctionList?num=${startPageNum - 1}">Previous</a></li>
					</c:if>
					...
					<c:forEach begin="${startPageNum}" end="${endPageNum}" var="num">
						...
						<c:if test="${select!=num}">
							<li class="page-item"><a class="page-link"
								href="/admin/underwayAuctionList?num=${num}">${num}</a></li>
						</c:if>

						<c:if test="${select==num}">
							<a class="page-link" href="/admin/underwayAuctionList?num=${num}">${num}</a>
						</c:if>
					...
					<c:if test="${next}">
						<li class="page-item"><a class="page-link"
							href="/admin/underwayAuctionList?num=${endPageNum + 1}">Next</a></li>
					</c:if>
				</ul>
